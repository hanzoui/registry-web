    ...
    useAsyncFn(async () => {
      ...
      await pageFlow(1, async (page, limit = 100) => {
        ...
        .map(async (node) => {
          ...
          node.latest_version || DIES(toast.error, `missing latest_version${JSON.stringify(node)}`);
          node.latest_version?.version ||
            DIES(toast.error, `missing latest_version.version${JSON.stringify(node)}`);
  ...
  useAsync(async () => {
    ...
    await pageFlow(1, async (page, limit = 100) => {
      ...
      .then((e) => {
        ...
        // outdated
        console.log(`got ${e.filter((x) => x.isOutdated).length} outdated results`);
    ...
    <div className="py-4 max-w-full relative dark">
      <div className="py-4">
        <Breadcrumb>
          ...
          <Breadcrumb.Item className="dark">{t("Node Version Compatibility")}</Breadcrumb.Item>
        </Breadcrumb>
      </div>
      ...
      >
        <div className="flex items-center">
          ...
          <TextInput
            ...
            defaultValue={nodeId && version ? `${nodeId}@${version}` : nodeId ? `${nodeId}` : ""}
          />
        </div>
        <div className="flex items-center">
...
function DataTable({
  ...
  const versions = useMemo(
    () => data?.versions?.filter((v) => (!version ? true : v.version === version)) || [],
    [data?.versions],
  );
        ...
        <Table.Body className="max-w-full overflow-auto">
          {versions?.map((nv) => {
            ...
            const compatibilityInfo = latestVersion ? (
              <div className="text-sm w-[50vw] z-100 ml-4">
                ...
                <div className="space-y-1">
                  ...
                  <div>
                    ...
                    {node.supported_accelerators?.join(", ") || t("Not specified")}
                  </div>
                </div>
                {isLatest && (
              ...
              <div className="text-sm">{t("Latest version information not available")}</div>
            );
              ...
              <Table.Row key={nv.id}>
                <Table.Cell className="sticky left-0 z-10 bg-gray-800 flex flex-row gap-2 justify-between">
                  ...
                  <div className="flex flex-row gap-2">
                    {isOutdated && (
                      <span
                        ...
                        onClick={async (e) => {
                          ...
                          if (!latestVersion) DIES(toast.error, "No latest version");
                          if (!isLatest) DIES(toast.error, "Not the latest version");
                    ...
                    {latestVersion ? (
                      <Tooltip content={compatibilityInfo} className="max-w-md" placement="right">
                ...
                <Table.Cell>
                  <Button size="xs" onClick={() => handleEdit(nv)} color="primary">
                    {t("Edit")}
                  </Button>
                </Table.Cell>
              </Table.Row>
