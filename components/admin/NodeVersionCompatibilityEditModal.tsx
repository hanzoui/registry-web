  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      await adminUpdateNodeVersion.mutateAsync({
        nodeId: nodeVersion.node_id || DIES(toast.error.bind(toast), t("Node ID is required")),
        versionNumber:
          nodeVersion.version ||
    ...
    <Modal show={isOpen} onClose={handleClose} size="2xl" className="dark">
      ...
      >
        <Modal.Body>
          <div className="space-y-4">
            ...
            {nodeData?.latest_version &&
              nodeData.latest_version.version !== nodeVersion.version && (
                <Alert color="info" className="dark">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="font-semibold">
                        {t("Latest Version Compatibility Reference")}:{" "}
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          if (latestVersion) {
                            ...
                            toast.success(t("Copied compatibility settings from latest version"));
                          }
                        }}
                      >
                        {t("Copy from Latest")}
                    ...
                    <div className="text-sm space-y-1">
                      ...
                      {nodeData.latest_version.supported_os &&
                        nodeData.latest_version.supported_os.length > 0 && (
                          <div>
                            <span className="font-medium">{t("OS")}:</span>{" "}
                            {nodeData.latest_version.supported_os.join(", ")}
                          </div>
                        )}
                      {nodeData.latest_version.supported_accelerators &&
                        nodeData.latest_version.supported_accelerators.length > 0 && (
                          <div>
            ...
            {nodeData?.latest_version &&
              nodeData.latest_version.version === nodeVersion.version && (
                <Alert color="success" className="dark">
                  <div className="font-semibold">{t("This is the latest version")}</div>
            ...
            <div>
              <Label htmlFor="supported-os">{t("Supported Operating Systems")}</Label>
            ...
            <div>
              <Label htmlFor="supported-accelerators">{t("Supported Accelerators")}</Label>
              <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">
        ...
        <Modal.Footer className="flex justify-end space-x-2">
          ...
          >
            {adminUpdateNodeVersion.isPending ? t("Saving...") : t("Save Changes")}
          </Button>
        </Modal.Footer>
      </form>
    </Modal>
